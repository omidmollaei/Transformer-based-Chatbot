"""
This module helps to load and prepare the Cornell Movie Dialog
dataset which is a famous dataset for training an open-ended
chatbot.
"""
...
def preprocess_sentence(sentence: str)-> str:
    """Clean the input sentence by adding whitespace between
    punctuations and characters and normalize the sentence."""
    sentence = sentence.lower().strip()

    # create a space between a word and the punctuation follwoing it.
...
def load_conversations(hparams: HyperParameters,
                       lines_filename: str,
                       conversations_filename: str)-> Tuple[List, List]:
    """this function loads the conversations from cornel dialog dataset. It works
    based on structure of this specific dataset and can not be used for other datasets."""
    ...
    for line in tqdm(lines):
        ...
        for i in range(len(conversation) - 1):
            ...
            if len(questions) >= hparams.max_sample:
                return questions, answers
    return questions, answers


def tokenize_and_filter(hparams: HyperParameters,
                        tokenizer: tokenizer_type,
                        questions: List,
                        answers: List) -> Tuple[List, List]:
    """this functions converts the clean text into tokens using
        the given tokenizer."""
...
def get_dataset(hparams: HyperParameters) -> Tuple[tf.data.Dataset, tokenizer_type]:
